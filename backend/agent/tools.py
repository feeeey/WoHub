...
def run_screener_scan(screener_keys: list[str], timeframes: list[str],
    ...
    progress_cb 的异常会原样向外传播（取消机制依赖此行为，勿捕获）。"""
    from sources import pine_screener as ps
    try:
        valid = {f"{s['folder_type']}/{s['screener_name']}": s["label"]
                 for s in ps.list_screeners()}
    except Exception as e:
        return {"error": f"筛选器清单加载失败: {e}"}
    ...
    try:
        cross = ps.build_cross_analysis(results)
    except Exception as e:
        cross = {}
        errors.append({"key": "_cross", "resolution": "-", "error": f"交叉分析失败: {e}"})
    return {"results": results, "cross": cross, "errors": errors,
